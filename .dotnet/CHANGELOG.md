...
- The library now includes support for the new [OpenAI o1](https://openai.com/o1/) model family.
  - `ChatCompletionOptions` will automatically apply its `MaxTokens` value to the new `max_completion_tokens` request body property
  - `Usage` includes a new `ReasoningTokens` property that will reflect `o1` model use of this new subcategory of output tokens.
	- Note that `OutputTokens` (`completion_tokens`) is the *sum* of displayed tokens generated by the model *and* (when applicable) these new reasoning tokens
