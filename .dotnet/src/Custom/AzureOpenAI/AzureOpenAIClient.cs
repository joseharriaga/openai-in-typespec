...
public partial class AzureOpenAIClient : OpenAIClient
{
    ...
    /// <remarks>
    /// <para>
    /// For token-based authentication, including the use of managed identity, please use the Azure.AI.OpenAI
    /// package.
    /// </para>
    /// </remarks>
    /// <param name="endpoint">
    ...
    public AzureOpenAIClient(Uri endpoint, string apiVersion, ApiKeyCredential credential, AzureOpenAIClientOptions options = null)
        : this(
    ...
    protected AzureOpenAIClient(ClientPipeline pipeline, Uri endpoint, string apiVersion, AzureOpenAIClientOptions options)
   ...
   => throw new NotImplementedException();
    ...
    public override BatchClient GetBatchClient()
        => throw new NotImplementedException();
        //=> GetBatchClient(deploymentName: null);
    ...
    public override ChatClient GetChatClient(string deploymentName)
        => new AzureChatClient(Pipeline, Endpoint, deploymentName, _apiVersion, _options);
    ...
    {
        string environmentApiVersion = Environment.GetEnvironmentVariable(s_aoaiApiVersionEnvironmentVariable);
        if (string.IsNullOrEmpty(environmentApiVersion))
        {
            throw new InvalidOperationException(
                $"No environment variable value was found for OPENAI_API_VERSION. "
                    + "Please either populate this environment variable or provide endpoint information directly "
                    + "to the client constructor.");
        }
    ...
    private static PipelinePolicy CreateAddUserAgentHeaderPolicy(AzureOpenAIClientOptions options = null)
    {
        return new GenericActionPipelinePolicy(message =>
        {
            ...
            {
                message.Request.Headers.Set(s_userAgentHeaderKey, "to-do");
            }
        });
    }
    ...
    private static readonly string s_aoaiApiVersionEnvironmentVariable = "OPENAI_API_VERSION";
