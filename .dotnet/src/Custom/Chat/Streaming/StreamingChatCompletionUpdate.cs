...
public partial class StreamingChatCompletionUpdate
{
    ...
    private ChatMessageContent GetWrappedContentUpdate()
    {
        ChatMessageContent wrappedResult = new();

        if (InternalChoiceDelta?.Audio is not null)
        {
            wrappedResult.Add(new ChatMessageContentPart(
                ChatMessageContentPartKind.Audio,
                responseAudioUpdate: InternalChoiceDelta.Audio));
        }
        foreach (ChatMessageContentPart deltaContentPart in InternalChoiceDelta?.Content ?? [])
        {
            wrappedResult.Add(deltaContentPart);
        }
